...
@Component({
  selector: 'app-reservation-cancelled-notification',
  standalone: true,
  imports: [
    DatePipe,
    RouterModule,
  ...
  template: `
    @if(reservation) {
      Prenotazione annullata per {{reservation.people}} persone per {{reservation.datetime | date: 'dd/MM/yyyy HH:mm'}} a nome {{reservation.fullname}}
...
export class ReservationCancelledNotificationComponent {
